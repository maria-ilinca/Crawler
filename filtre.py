...
Iasi=0
Constanta=0
Brasov = Cluj = Timisoara = 0

for element in lista_descriere:
    if re.findall('(?i)Bucuresti', element[0]) or re.findall('(?i)Bucuresti', element[1]):
        Bucuresti+=1
    if re.findall('(?i)Iasi', element[0]) or re.findall('(?i)Iasi', element[1]):
        Iasi+=1
    if re.findall('(?i)Brasov', element[0]) or re.findall('(?i)Brasov', element[1]):
       Brasov+=1
    if re.findall('(?i)Cluj', element[0]) or re.findall('(?i)Cluj', element[1]):
       Cluj+=1
    if re.findall('(?i)Timisoara', element[0]) or re.findall('(?i)Timisoara', element[1]):
       Timisoara+=1
    if re.findall('(?i)Constanta', element[0]) or re.findall('(?i)Constanta', element[1]):
       Constanta+=1

f.write(f"\nLocuri de munca in Bucuresti: {Bucuresti}")
f.write(f"\nLocuri de munca in Iasi: {Iasi}")
f.write(f"\nLocuri de munca in Constanta: {Constanta}")
f.write(f"\nLocuri de munca in Timisoara: {Timisoara}")
f.write(f"\nLocuri de munca in Cluj: {Cluj}")
f.write(f"\nLocuri de munca in Brasov: {Brasov}")

# f.write(lista_descriere)


f.write(f"\n---------------------------")
f.write(f"\n10.Program in weekend:")

weekend=0

for element in lista_descriere:
    if re.findall('(?i)s[aăîâ]mb[aăâ]t[aăâ]|duminic[aăâ]|program în weekend|program în week-end', element[0]) or re.findall('(?i)s[aăîâ]mb[aăâ]t[aăâ]|duminic[aăâ]|program în weekend|program în week-end', element[1]):
        weekend+=1
...
f.write(f"\n---------------------------")
f.write(f"\n11.Bonusuri salariale:")

bonus=bon_de_masa=0
for element in lista_descriere:
    if re.findall('(?i)bonus|bonusuri', element[0]) or re.findall('(?i)bonus|bonusuri', element[1]):
        bonus+=1
    if re.findall('(?i)bonuri de masa|tichete de masa', element[0]) or re.findall('(?i)bonuri de masa|tichete de masa', element[1]):
        bon_de_masa+=1

f.write(f"\nBonuri de masa: {bon_de_masa}")
f.write(f"\nBonusuri: {bonus}")
...
for lista in profesii:
    for nume in lista:
        if len(nume)>=4 and nume!="urgent" and nume!="ajutor" and nume!="ajutori" and nume!="femeie" and nume!="femei" and nume!="baiat" and nume!="baieti" and nume!="fata" and nume!="doamna" and nume!="coleg" and nume!="persoane" and nume!="colegi" and nume!="oameni" and nume!="fete":
            prof.append(nume.lower())

alfabet=['q','w','e','r','t','y','u','i','o','p','l','k','j','h','g','f','d','s','a','z','x','c','v','b','n','m']
for x in prof:
    if x not in prof_unice:
        if x[0] not in alfabet :
            if x[1::] not in prof_unice:
                prof_unice.append(x[1::])
        elif x[len(x)-1]=="i":
            if x[:len(x)-1:] not in prof_unice:
                prof_unice.append(x[:len(x)-1:])
        else:
            prof_unice.append(x)

# f.write(str(prof_unice))
# contorizare profesii
for serviciu in prof_unice:
    c=0
    for element in lista_descriere:
        if element[0].count(serviciu)!=0 or element[1].count(serviciu)!=0:
            c+=1
    f.write(f"\n{serviciu}: {c}")
